...
def test_every_page_uses_the_shared_page_shell(app: Callable, path: str) -> None:
    ...
    assert b"Version 0.3.3+dev" in body
...
def test_about_page_is_served_with_active_navigation(app: Callable) -> None:
    ...
    assert b"Version 0.3.3 is a fast reference" in body
...
def test_assets_and_catalog_api_have_release_safe_cache_headers(app: Callable) -> None:
    status, headers, _ = request(app, "/static/styles.css?v=0.3.3")
    assert status == 200
...
def test_versioned_modules_reference_their_matching_release_dependencies(app: Callable) -> None:
    status, headers, body = request(app, "/static/unit.js?v=0.3.3")

    assert status == 200
    assert headers["cache-control"] == "public, max-age=31536000, immutable"
    assert b'from "./api.js?v=0.3.3"' in body
    assert b'from "./preferences.js?v=0.3.3"' in body

    status, _, body = request(app, "/static/api.js?v=0.3.3")
    assert status == 200
    assert b'import("./preferences.js?v=0.3.3")' in body
