...
- [x] Enforce Army snapshot acquisition coherence without assuming a single
  Corvus Belli source revision. The downloader now performs two complete passes
  over metadata and every Army/reinforcement endpoint, requires byte-identical
  responses before writing snapshot files, reports changed endpoint hashes on a
