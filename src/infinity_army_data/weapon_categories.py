...
from .weapon_config import WeaponCategoryConfig, load_weapon_category_config
...
def weapon_category(
    ...
    *,
    config: WeaponCategoryConfig = _CONFIG,
) -> str:
    """Classify a weapon name using maintained policy and generic matching mechanics."""

    if weapon_id is not None and weapon_id in config.overrides:
        return config.overrides[weapon_id]
    text = str(name or "")
